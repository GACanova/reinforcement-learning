...
class Blackjack:
    """
    A Blackjack game simulator implementing the Exploring Starts Monte Carlo method for Reinforcement Learning, as described in Sutton and Barto's book.

    This simulator models the game of Blackjack, where the goal is to maximize the sum of card values without exceeding 21. It uses the Exploring Starts Monte Carlo method to 
    estimate state-action values and develop an optimal playing policy. This is crucial for understanding the strategic aspects of Blackjack and for determining the best course of 
    action in various game states.

    Attributes:
        gamma (float): Discount factor for future rewards.
        actions (list): Possible actions in the game, represented as [0, 1] for hit and stick.
        state_action_fuction (numpy array): Table for storing state-action values.
        policy (numpy array): Table representing the policy, mapping states to actions.
        n_visited (defaultdict): A dictionary tracking the number of times a state-action pair has been visited.
    ...
    Methods:
        initialize_state_action(): Initializes the state-action function.
        initialize_policy(): Initializes the policy randomly.
        generate_action(): Generates a random action.
        ...
        exploring_starts_mc(n_episodes=100000): Runs the Exploring Starts Monte Carlo algorithm for the specified number of episodes.
    """
    ...
    def initialize_state_action(self):
        self.state_action_fuction = np.zeros((22, 11, 2, 2))
            ...
            for dealer_card in range(0, 11):
                for usable_ace in [0, 1]:
                    ...
                    for action in self.actions:
                        action_values.append(self.state_action_fuction.get((state, action), 0))
    ...
    def select_greedy_action(self, state):
        ...
        for action in self.actions:
            action_values.append(self.state_action_fuction[(state, action)])
        
        action_values = np.array(action_values)
        best_action = self.actions[np.argmax(np.random.random(action_values.shape[0]) * (action_values == action_values.max()))]
    ...
    def exploring_starts_mc(self, n_episodes=100000):
        ...
        self.state_action_fuction = defaultdict(float)
        self.n_visited = defaultdict(int) 
        ...
        for i in range(0, n_episodes):            
            ...
            for t, (state, action, reward) in reversed(list(enumerate(episode))):
                ...
                if not (state, action) in [(x[0], x[1]) for x in episode[:t]]:
                    ...
                    self.state_action_fuction[(state, action)] += (G - self.state_action_fuction[(state, action)]) / (self.n_visited[(state, action)])
                    self.policy[state] = self.select_greedy_action(state)
                    
        return self.state_action_fuction, self.policy
